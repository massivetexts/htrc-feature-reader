...
class IdResolver():
    ...
    def fname(self, id, format, compression, suffix):
        ...
        if format == "parquet" or (format == "feather" and compression != "gz"):
...
def make_resolver_chain(dicts):
    """
    Given a list of dictionaries, returns a resolver that
    """
    import copy
    dicts = [copy.deepcopy(dict) for dict in dicts]
    for dict in dicts:
